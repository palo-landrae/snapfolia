...
async def detect(file: UploadFile):
    """
    Upload an image and send it to the detection worker via Celery.
    ...
    if cached_result is not None:
        response = DetectionResponse.model_validate(cached_result)
    ...
    # Send Celery task to detection queue

    task = celery.send_task(
        "detect_objects_task", args=[str(file_path)], queue="detection"
    )
