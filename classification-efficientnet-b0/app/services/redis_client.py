...
class RedisClient:
    ...
    def push_to_stream(
        self, stream_name: str, data: Any, max_len: int = 10000
    ) -> Optional[str]:
        """
        Pushes data to a Redis Stream.
        Uses Dict[Any, Any] to bypass TypeVar invariance issues in redis-py.
        """
        try:
            # Construct the payload
            payload: Dict[Any, Any] = {"data": json.dumps(data)}

            # Use the payload in xadd
            msg_id = self.client.xadd(
                name=stream_name, fields=payload, maxlen=max_len, approximate=True
            )
